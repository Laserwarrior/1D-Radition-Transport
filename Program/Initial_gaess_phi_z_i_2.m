function [phi_z_0, phi_z_1] = Initial_gaess_phi_z_i_2(psi_boundary, L_plus_T_positive, L_plus_T_negative, Spatial_Material_Parameter, Angle_positive, Weight_positive, Angle_negative, Weight_negative, tau_z, Spatial_discretization_option)
...
for i = 1: length(Angle_positive)
    ...
    [phi_z_0_negative(:, i), phi_z_1_negative(:, i)] = sovle_matrix_not_known_phi(Angle_negative(i), flip(Spatial_Material_Parameter), tau_reverse(:, i), psi_boundary(i,2), L_plus_T_negative(:, :, i), 1, Weight_negative(i), Spatial_discretization_option);
end
    phi_z_0 = sum(phi_z_0_positive, 2) + sum(flip(phi_z_0_negative), 2);
    phi_z_1 = sum(phi_z_1_positive, 2) + sum(flip(phi_z_1_negative), 2);
